...
object RetrieveTrustDetailsHttpParser {

  implicit object RetrieveTrustDetailsHttpReads extends HttpReads[Option[TrustDetails]] {
  ...
  def reads(json: JsValue): JsResult[TrustDetails] = {
    for {
      ...
      val optSautr = if (optUtr.nonEmpty) Some(Sautr(optUtr.get)) else None
      TrustDetails(optSautr, optPostcode, optChrn, optIdentifiersMatch, optBusinessVerificationStatus, optRegistrationStatus)
    }
  }
