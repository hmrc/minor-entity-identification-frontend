...
import uk.gov.hmrc.minorentityidentificationfrontend.assets.MessageLookup.{Base, BetaBanner, Header, CaptureOverseasTaxIdentifiersCountry => messages}
...
trait CaptureOverseasTaxIdentifiersCountryTests {
  this: ComponentSpecHelper =>

  def testCaptureOverseasTaxIdentifiersCountryView(result: => WSResponse): Unit = {
  ...
  def testCaptureOverseasTaxIdentifiersCountryErrorMessages(result: => WSResponse): Unit = {
    lazy val doc: Document = Jsoup.parse(result.body)
    ...
    "correctly display the error summary" in {
      ...
      doc.getErrorSummaryBody.get(0).text mustBe messages.Error.no_input_country
    }

    "correctly display the field errors" in {
      doc.getFieldErrorMessage.get(0).text mustBe Base.Error.error + messages.Error.no_input_country
    }
  }
